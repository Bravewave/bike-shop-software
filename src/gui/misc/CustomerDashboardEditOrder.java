...
public class CustomerDashboardEditOrder {
    ...
    public static JFrame editOrder(Order order, GUIFrame guiFrame) {
        ...
        try {
            ...
            if (order.getStaffID() == -1) {
                staffIDField = new JTextField("No Staff Assigned");
            } else {
                staffIDField = new JTextField("Staff Member Not Found");
            }
        ...
        cancelOrderButton.addActionListener(e -> {
            try {
                if (Utilities.deleteOrder(order.getOrderID(), null)) {
                    JOptionPane.showMessageDialog(frame, "Order cancelled successfully.");
                }
                else {
                    JOptionPane.showMessageDialog(frame, "Order could not be cancelled.");
                }
                frame.dispose();
        ...
        frame.add(new CustomerDashboardEditOrder().constructTable(order.getBike()), BorderLayout.CENTER);
